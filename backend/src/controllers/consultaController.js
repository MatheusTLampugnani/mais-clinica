...
const consultaController = {
  ...
  async getFilaDeEspera(req, res) {
    try {
      ...
      const fila = await Consulta.findAll({
        where: {
          dataHora: { [Op.gte]: hoje, [Op.lt]: amanha },
          status: 'agendada',
        },
        include: [
          { model: Paciente, attributes: ['nome'] },
  ...
  async getByMedico(req, res) {
    try {
      ...
      const medico = await Medico.findOne({ where: { UsuarioId: usuarioId } });
      ...
      const consultas = await Consulta.findAll({
        where: { MedicoId: medico.id }, // Usando o ID correto agora
  ...
  async getByPaciente(req, res) {
    try {
      const usuarioId = req.params.id;
      const paciente = await Paciente.findOne({ where: { UsuarioId: usuarioId } });
      ...
      const consultas = await Consulta.findAll({
        ...
        include: [
          ...
          { model: Prontuario, include: [ AnexoExame ] }
        ],
        order: [['dataHora', 'DESC']]
      });
